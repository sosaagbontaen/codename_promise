...
/// The models exactly as v4 shipped them.
///
/// Frozen because `SchemaV4` used to point at the live classes, which was correct only while
/// it was the newest version. v5 exists now, and a version still naming the live types
...
///
/// **Derived from the v3 copy plus exactly what v4 added**, rather than retyped from the live
/// models: v4 is v3 with `SyncState.photoBlockIds` and `SyncState.videoBlockIds`, and nothing
/// else. SwiftData matches a store to a version by checksum, so a plausible shape that is off
/// by one attribute fails exactly as loudly as no shape at all.
///
/// Relative to v5, v4 is missing precisely one thing, on `MediaItem`:
///
/// - `partRelativePaths`
///
/// An array with an empty default, which is what keeps the migration lightweight.
extension SchemaV4 {
    ...
    public final class EntryDraft {
        ...
        @Relationship(deleteRule: .cascade, inverse: \SchemaV4.MediaItem.draft)
        public var media: [SchemaV4.MediaItem] = []

        @Relationship(deleteRule: .cascade, inverse: \SchemaV4.AudioCapture.draft)
        public var audioCaptures: [SchemaV4.AudioCapture] = []

        @Relationship(deleteRule: .cascade, inverse: \SchemaV4.SyncState.draft)
        public var syncStates: [SchemaV4.SyncState] = []
    ...
    public final class MediaItem {
        ...
        @Relationship public var draft: SchemaV4.EntryDraft?
    ...
    public final class SyncState {
        ...
        // What v4 added. The words are written before the media now, in three calls, and the
        // server deletes exactly the block ids it is handed, so each stage tracks its own.
